...
// \TODO Put your Next Fit code in this #ifdef block
   ...
   while (curr && curr->next != nfPtr && nfPtr !=NULL) 
   {
       curr  = curr->next;
       ...
       if(curr->free)
       {
         if(curr->size > size)
         {
            if(curr->next ==NULL && curr==heapList)
            {
               nfPtr = curr;
            }
            else
...
void *malloc(size_t size) 
{
   ...
   */
   if(next)
   {
      if((int)(next->size)>(int)size)
      {
         size_t new_size=(next->size)-size; 

         if(new_size > sizeof(struct _block) + 4)
         {
            new_node = (struct _block *)((char *)(next) + sizeof(struct _block) + size);
            
            new_node->size = next->size -  sizeof(struct _block) - size;
            next->size = size;
